// ===== Field factory =====
export const f = (value, coluna, transform = "Direta", linha = 42, candidatos = null) => ({
  value,
...
export const MONO = new Set(["cnpj", "cpf", "titularCpf", "matricula", "competencia", "dtNasc", "admissao"]);
...
// ===== Record factory =====
export function rec(id, tipo, status, cpKey, vida, campos, validacoes, extra = {}) {
  const base = {
  ...
  return {
    id, batchId: "LT-2026-0619-A", tipo, status, destino: CP[cpKey], vida,
    campos: {
      ...base, ...campos,
    ...
    validacoes,
    timeline: [
      { id: id + "-t0", origem: "sistema", tipo: "estado", autor: "Captura", quando: "19/06 08:12", texto: "Movimentação capturada e normalizada ao modelo canônico." },
      ...(status === "aprovado" ? [{ id: id + "-t1", origem: "humano", tipo: "estado", autor: ANALISTA, quando: "19/06 08:30", texto: "Movimentação aprovada." }] : [])
    ],
...
const vCpf        = { dim: "captura",        sev: "erro",  msg: "CPF inválido — falhou no dígito verificador.", campo: "cpf" };
const vMae        = { dim: "captura",        sev: "aviso", msg: "Nome da Mãe ausente na origem.", campo: "nomeMae" };
const vAmbig      = { dim: "captura",        sev: "aviso", msg: "Data de Nascimento inferida de coluna ambígua — confirme a origem.", campo: "dtNasc" };
const vTitularCore= { dim: "movimentacao",   sev: "erro",  msg: "Titular não localizado como ativo.", core: "Consulta ao Core: VIDA do titular não encontrada como vínculo ativo." };
const vExclCore   = { dim: "movimentacao",   sev: "erro",  msg: "Vida não localizada como ativa para exclusão.", core: "Consulta ao Core: VIDA-00831 sem vínculo ativo na operadora." };
const vPlano      = { dim: "movimentacao",   sev: "erro",  msg: "Plano não habilitado para este CNPJ no contrato vigente." };
const vIdade      = {
...
// ===== Display helpers =====
export const tipoCls = { "Inclusão": "tipo-inc", "Exclusão": "tipo-exc", "Alteração": "tipo-alt" };
...
export const hasErro  = r => r.validacoes.some(v => v.sev === "erro");
export const hasAviso = r => r.validacoes.some(v => v.sev === "aviso");
export const now      = () => "19/06 " + new Date().toTimeString().slice(0, 5);
